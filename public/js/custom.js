...
document.getElementById("about").className = "collapse"
};
...
function success(pos) {
  ...
  $.get("https://maps.googleapis.com/maps/api/geocode/json?latlng="+crd.latitude+","+crd.longitude+"&sensor=false", function(data, status){
       allAddyInfo = data;
...
function getGovtInfo(addy){
  //  addy = addy.replace(/,/g , "%");
  clearResults()
   addy = "https://www.googleapis.com/civicinfo/v2/representatives?key=AIzaSyCS7dd6uTeV4wfO-aBbdpln7tqJvxU546U&address=" + addy
   $.get(addy, function(data, status){
     ...
     function getWebsite(){
       if(data.officials[i].urls && data.officials[i].urls[0]){
         ...
         currentPres.website = `<a target="_blank" class="btn btn-primary" href="`+currentPres.website+`">Visit Website</a>`;
         return currentPres.website;
         } else {return "<br>"}
      };

      function getParty(){if (data.officials[i].party) {
        currentPres.party = data.officials[i].party;
        if (currentPres.party != 'undefined'){
        return currentPres.party;
        }
      } else {return "Unknown Political Association"
      };
      };

      function getTitle(){try {
        if (data.offices[i].name){
        currentPres.title = data.offices[i].name;
        return currentPres.title;
      }
      } catch(e) {return "City Official"
      };
      };

      function getName(){try {
      ...
      function getUrl(){
          try {
                if (data.officials[i].photoUrl[0]){
                    ...
                    <img id="0Pic" width="300" height="300"class="img-responsive" src="` + currentPres.pic + `" alt="">
                    </a>`)
...
if(getName()){
        ...
        <div class="row">
            ...
            <div class="col-md-5">
                <h3 id="0Name">`+ getName() +`</h3>
                <h4 id="0Title">`+ getTitle() +`</h4>
                <h4 id="0Title">`+ getNumber() +`</h4>
                <h4>`+ getParty() +`
                <h4>`+ getCity() +`</h4>
                <h4>`+ getLine1() +`</h4>
                `+ getLine2() +`
      ...
      `);
       ...
       document.getElementById("representativeResults").innerHTML = document.getElementById("representativeResults").innerHTML + thisThing;
